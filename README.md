...
- 14개의 재사용 가능한 Terraform 모듈로 구성되며, 3개 환경(dev/staging/prod)별 설정을 지원합니다.
...
- CI/CD 파이프라인 자동화 (GitHub Actions 등)
- 멀티 리전 배포
- Kubernetes/ECS 컨테이너 오케스트레이션
- Terraform 원격 상태 관리 (S3 + DynamoDB 백엔드)
...
- **Terraform 상태 관리**: 현재 로컬 파일(`terraform.tfstate`)로 관리. 팀 협업 시 S3 + DynamoDB 백엔드로 전환 권장.
