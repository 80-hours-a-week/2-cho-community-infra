...
│
├── environments/               # 환경별 설정
│   ├── bootstrap/              # 상태 백엔드 부트스트랩 (로컬 상태, 1회 실행)
│   ├── dev/
...
S3 버킷과 DynamoDB 테이블을 먼저 생성해야 각 환경에서 원격 백엔드를 사용할 수 있습니다.
...
- **부트스트랩 상태**: `environments/bootstrap/`은 영구 로컬 상태 사용. S3 백엔드로 전환 불가 (자기 자신을 저장할 버킷이 아직 없음)
